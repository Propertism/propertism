...
|---|---|---|:---:|:---:|---|
| **TSK-MIG-01** | **Provision Lightsail Instance & Static IP** | AWS Lightsail (`ap-south-1`) | Viji / Astra | ⚪ READY | **Target**: Ubuntu 24.04 LTS, 2 GB RAM / 2 vCPU / 60 GB SSD / 3 TB Transfer ($12/mo tier).<br>**Evidence**: Static IPv4 attached; ports 22, 80, 443 open. |
| **TSK-MIG-02** | **OS Hardening, Swap & Package Stack Installation** | Host OS / Linux | Astra | ⚪ READY | **Action**: 2 GB swap file enabled (`vm.swappiness=10`); UFW active (`OpenSSH`, `Nginx Full`); packages installed: Nginx, PostgreSQL 16, Redis 7, Python 3, python3-venv, Certbot.<br>**Evidence**: `swapon --show`, `ufw status`, `psql --version`, `redis-cli ping` returns `PONG`. |
